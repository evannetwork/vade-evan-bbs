...
impl VadePlugin for VadeEvanBbs {
    ...
    async fn vc_zkp_issue_credential(
        ...
        let public_key: DeterministicPublicKey = DeterministicPublicKey::from(
            decode_base64(&payload.issuer_public_key)?.into_boxed_slice(),
        );
        let sk: SecretKey =
            SecretKey::from(decode_base64(&payload.issuer_secret_key)?.into_boxed_slice());
    ...
    async fn vc_zkp_present_proof(
        ...
        let master_secret: SignatureMessage =
            SignatureMessage::from(decode_base64(&payload.master_secret)?.into_boxed_slice());
    ...
    async fn vc_zkp_request_credential(
        ...
        let master_secret: SignatureMessage =
            SignatureMessage::from(decode_base64(&payload.master_secret)?.into_boxed_slice());
        let public_key: DeterministicPublicKey = DeterministicPublicKey::from(
            decode_base64(&payload.issuer_pub_key)?.into_boxed_slice(),
        );
    ...
    async fn vc_zkp_finish_credential(
        ...
        let blinding: SignatureBlinding =
            SignatureBlinding::from(decode_base64(&payload.blinding)?.into_boxed_slice());
        let master_secret: SignatureMessage =
            SignatureMessage::from(decode_base64(&payload.master_secret)?.into_boxed_slice());
